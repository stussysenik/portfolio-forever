 ...
 *
 * Refactored as a factory: createCommandRegistry(data) injects Convex-backed
 * data while keeping all hardcoded values as defaults. Command LOGIC stays
 * client-side; command DATA becomes data-driven.
 */
...
// Data injected from Convex (all optional — falls back to defaults)

export interface TerminalData {
  fortunes?: string[];
  asciiLogo?: string;
  neofetchFields?: { label: string; value: string }[];
  whoamiOutput?: string;
...
const DEFAULT_ASCII_LOGO = `<span class="t-accent">  ███████╗████████╗██╗   ██╗</span>
...
const DEFAULT_WHOAMI_OUTPUT = `<span class="t-info">stussysenik</span> <span class="t-muted">— Design Engineer & Creative Producer, Bed-Stuy BK</span>`;

const DEFAULT_NEOFETCH_FIELDS = [
...
const DEFAULT_PROJECT_URLS: Record<string, { url: string; title: string }> = {
...
export function createCommandRegistry(data: TerminalData = {}) {
  // Merge injected data with defaults
  const fortunes = data.fortunes ?? DEFAULT_FORTUNES;
  const asciiLogo = data.asciiLogo ?? DEFAULT_ASCII_LOGO;
  const whoamiOutput = data.whoamiOutput ?? DEFAULT_WHOAMI_OUTPUT;
  const neofetchFields = data.neofetchFields ?? DEFAULT_NEOFETCH_FIELDS;

  // Build project URL map from injected data or defaults
  const urlMap: Record<string, { url: string; title: string }> = { ...DEFAULT_PROJECT_URLS };
  if (data.projectUrls) {
    for (const p of data.projectUrls) {
      urlMap[p.name.toLowerCase()] = { url: p.url, title: p.name };
    }
  }

  // Build skills list from injected data or defaults
  const skills = data.skills
    ? data.skills.map((s, i) => ({
        pid: String(i + 1).padStart(3, '0'),
        name: s.name,
        cpu: s.proficiency,
        status: s.proficiency >= 70 ? 'running' : 'learning',
      }))
    : DEFAULT_SKILLS;

  // Build packages list from injected data or defaults
  const packages = data.packages
    ? data.packages.map(p => `${p.name}@${p.version}`)
    : DEFAULT_PACKAGES;
    ...
    neofetch: {
      ...
      fn: () => {
        ...
        const info = [
          ...
          ...neofetchFields.map(f =>
            `<span class="t-accent">${pad(f.label, 9)}</span> <span class="t-muted">${f.value}</span>`
          ),
          `<span class="t-accent">${pad('Uptime', 9)}</span> <span class="t-muted">${uptimeDays} days (since Jan 2026)</span>`,
          `<span class="t-accent">${pad('Repos', 9)}</span> <span class="t-muted">${githubProfile.publicRepos} public</span>`,
          ``,
...

{
  ...
  function getCommandsMap(): Record<string, CommandDef> {
    return commands;
  }

  function executeCommand(input: string, state: ShellState): OutputLine[] {
  ...
  function getCompletions(input: string, state: ShellState): string[] {
  ...
  return {
    executeCommand,
    getCompletions,
    getCommands: getCommandsMap,
    getCommandNames,
  };
}

// ── Backward-Compatible Exports ───────────────────────────
// These use the default registry so existing consumers work unchanged.

const defaultRegistry = createCommandRegistry();

export function getCommands(): Record<string, CommandDef> {
  return defaultRegistry.getCommands();
}

export function executeCommand(input: string, state: ShellState): OutputLine[] {
  return defaultRegistry.executeCommand(input, state);
}

export function getCommandNames(): string[] {
  return defaultRegistry.getCommandNames();
}

export function getCompletions(input: string, state: ShellState): string[] {
  return defaultRegistry.getCompletions(input, state);
}
