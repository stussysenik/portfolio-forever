import { ColorHarmonyType } from "../types";
...
/**
 * Generate hue offsets for each harmony type.
 * Returns array of hue offsets from the base hue.
 */
function getHarmonyOffsets(type: ColorHarmonyType, count: number): number[] {
  switch (type) {
    case "complementary": {
      // Base + complement, then interpolate between
    ...
    case "split-complementary": {
      // Base + two colors flanking the complement (150deg and 210deg)
 ...
 * Targets minimum 3:1 for large text, 4.5:1 for body text.
 */
function enforceContrast(color: HSL, bgHex: string, minRatio: number = 3): HSL {
  let adjusted = { ...color };
...
/**
 * Generate a full set of 12 word-type colors from a base hue using the
 * specified color harmony type, with WCAG contrast enforced against the
...
export function generateHarmonyColors(
  ...
  // Vary saturation and lightness slightly per color for visual distinction
