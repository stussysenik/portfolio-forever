(ns portfolio.data.content
  "Portfolio content data — the canonical source of truth for static content.
   Ported from src/lib/data/content.ts
   This namespace provides all static portfolio data as Clojure data structures
   that compile to JS objects via Squint, keeping the Lisp expressiveness
   while producing identical runtime output.")

(def skill-categories #{"design" "technology" "art"})

(def entry-categories
...
(def skills
  [{:name "Motion Design" :category "design" :proficiency 0.85 :years-used 6}
   {:name "UX & Product Design" :category "design" :proficiency 0.75 :years-used 5}
   {:name "Visual Design" :category "design" :proficiency 0.9 :years-used 8}
   {:name "Sound Design" :category "design" :proficiency 0.7 :years-used 4}
   {:name "3D & CGI" :category "design" :proficiency 0.8 :years-used 5}
   {:name "Unreal Engine" :category "technology" :proficiency 0.75 :years-used 3}
   {:name "Rhino & Grasshopper" :category "technology" :proficiency 0.7 :years-used 4}
   {:name "WebGPU & WASM" :category "technology" :proficiency 0.8 :years-used 3}
   {:name "Three.js & WebGL" :category "technology" :proficiency 0.85 :years-used 5}
   {:name "Hardware & Sensors" :category "technology" :proficiency 0.65 :years-used 3}
   {:name "Creative Coding" :category "technology" :proficiency 0.9 :years-used 8}
   {:name "Art Direction" :category "art" :proficiency 0.85 :years-used 6}
   {:name "Filmmaking" :category "art" :proficiency 0.8 :years-used 5}
   {:name "Lighting Design" :category "art" :proficiency 0.75 :years-used 4}
   {:name "Digital Fabrication" :category "art" :proficiency 0.6 :years-used 2}
   {:name "Illustration" :category "art" :proficiency 0.7 :years-used 6}])
...
(def site-config
  {:name (:name profile)
   :title (:name profile)
   :tagline (-> profile :taglines first :text)})
...
(def social-links
  [{:label "soundcloud" :url "https://on.soundcloud.com/b7PpyyqCuScmugtNZc"}
   ...
   {:label "email" :url (str "mailto:" (:email profile))}])
...
(defn get-skills-by-category [category]
  (filterv #(= (:category %) category) skills))

(defn get-tools-by-category [category]
  (filterv #(= (:category %) category) tools))

(defn format-date [entry]
  (let [month (:month entry)]
    (if month
      (str (:year entry) "." (.padStart (str month) 2 "0"))
      (str (:year entry)))))

(def featured-color-map
  {"yellow" "1"
   "green" "2"
   "electric-green" "3"
   "orange" "4"
...
(defn get-highlight [entry]
  (when-let [featured (:featured entry)]
    (get featured-color-map featured)))

(defn sort-entries [entries]
  (sort-by (fn [entry]
              [(- (:year entry)) (-(or (:month entry) 0))])
            entries))

(def sorted-works (sort-entries works))
(def sorted-talks (sort-entries talks))
(def sorted-interviews (sort-entries interviews))
