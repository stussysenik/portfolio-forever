<script lang="ts">
  ...
  // Icons
  const icons = [
    { label: 'My Computer', icon: '💻', content: 'Nothing here yet.' },
    { label: 'Trash', icon: '🗑️', content: 'Empty.' },
    { label: 'Documents', icon: '📁', content: 'CV.pdf\nProjects.txt' },
    { label: 'Exit OS', icon: '🔌', action: 'exit' },
  ];
...
<svelte:head>
  ...
  <style>
    body {
        margin: 0;
        overflow: hidden;
        background-color: transparent; /* Let layout background show through or handle in .desktop */
    ...
    >
      <div class="title-bar" on:mousedown={(e) => onMouseDown(win.id, e)}>
        ...
        <div class="title-bar-controls">
          <button class="btn-close" on:click|stopPropagation={() => closeWindow(win.id)} aria-label="Close">×</button>
        </div>
      </div>
      <div class="content">
        <pre>{win.content}</pre>
...
<style>
  .desktop {
    width: 100vw;
    height: calc(100vh - 100px); /* Leave room for header/footer if needed, or just top offset */
    position: fixed;
    top: 100px; /* Offset for main navigation */
    left: 0; 
